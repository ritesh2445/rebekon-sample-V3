      ...
      <div className="animate-marquee flex items-center gap-5 sm:gap-6">
        {items.map((cert, index) => (
          <div
            ...
            className={`flex items-center gap-4 px-5 py-3.5 rounded-2xl bg-[#0E1424] border border-white/10 shadow-sm hover:border-[#007AFF]/60 hover:shadow-[0_12px_30px_-6px_rgba(0,122,255,0.2)] hover:-translate-y-0.5 transition-all duration-300 shrink-0 group ${
              onCardClick ? 'cursor-pointer' : 'cursor-default'
            }`}
            title={`${cert.name} • ${cert.issuer} (${cert.code})`}
          >
            {/* Logo Container */}
            <div className="h-10 sm:h-12 w-auto min-w-[75px] max-w-[125px] flex items-center justify-center shrink-0 p-1 bg-white/10 rounded-xl">
              <img
                src={cert.logoUrl}
                alt={cert.name}
                className="max-h-9 sm:max-h-11 w-auto max-w-[115px] object-contain brightness-0 invert opacity-80 group-hover:opacity-100 transition-all duration-300 group-hover:scale-105"
                loading="lazy"
              />
            </div>
            ...
            <div className="border-l border-white/10 pl-3.5 text-left">
              <div className="flex items-center gap-1.5">
                <span className="font-bold text-xs sm:text-sm text-slate-100 group-hover:text-[#007AFF] transition-colors whitespace-nowrap">
                  {cert.name}
                </span>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[10px] sm:text-[11px] font-mono font-bold text-[#007AFF] bg-[#007AFF]/15 px-2 py-0.5 rounded-md border border-[#007AFF]/30 whitespace-nowrap">
                  {cert.code}
                </span>
