              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <span className={`text-[11px] font-extrabold px-3 py-1 rounded-full border  ${getTagStyle(study.category)}`}>
                      {study.category}
                    </span>
                  ...
                  <h3 className="font-bold text-xl text-white group-hover:text-[#007AFF] transition-colors mb-4 leading-snug">
                    {study.title}
                  </h3>

                  <div className="space-y-3 mb-6 text-xs sm:text-sm text-slate-300">
                    <div>
                      <strong className="text-white block text-xs uppercase tracking-wider mb-1 font-bold">Challenge:</strong>
                      <p className="line-clamp-3 leading-relaxed">{study.challenge}</p>
                    </div>
                    <div>
                      <strong className="text-white block text-xs uppercase tracking-wider mb-1 font-bold">Solution:</strong>
                ...
                <button
                  ...
                  className="w-full py-3 rounded-full ios-btn-secondary text-xs font-bold flex items-center justify-center gap-1.5  cursor-pointer"
                >
      ...
      {activeModalStudy && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xl animate-in fade-in duration-200">
          <div className="ios-glass-card bg-[#0E1424] max-w-2xl w-full p-6 sm:p-10 max-h-[90vh] overflow-y-auto rounded-[2.5rem] shadow-2xl border border-white/15 relative">
            ...
            <h3 className="font-extrabold text-2xl sm:text-3xl text-white mb-6 leading-tight">
            ...
            <div className="space-y-6 text-slate-200 text-sm sm:text-base leading-relaxed">
              ...
              {activeModalStudy.quote && (
                <div className="p-6 rounded-2xl ios-glass border border-white/12 italic text-slate-100">
                  <p>"{activeModalStudy.quote}"</p>
                  {activeModalStudy.author && (
                    <span className="block text-xs font-bold not-italic text-slate-400 mt-2">
                      — {activeModalStudy.author}
                    </span>
                  )}
                </div>
