        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              ...
              <div className="p-8 rounded-3xl ios-glass-card shadow-md space-y-6">
                ...
                <div className="space-y-4 text-sm">
                  <a
                    ...
                    className="flex items-center gap-3 p-3 rounded-2xl bg-white/80 border border-white/90 hover:border-[#FF2D55] transition-colors text-slate-800 shadow-2xs"
                  >
                  ...
                  <a
                    ...
                    className="flex items-center gap-3 p-3 rounded-2xl bg-white/80 border border-white/90 hover:border-[#007AFF] transition-colors text-slate-800 shadow-2xs"
                  >
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-2xl bg-white/80 border border-white/90 text-slate-800 shadow-2xs">
                ...
                {/* What to expect card */}
                <div className="pt-4 border-t border-slate-200/60 space-y-2 text-xs text-slate-600 font-medium">
            ...
            <div className="lg:col-span-8">
              <div className="p-8 sm:p-12 rounded-[2.5rem] ios-glass-card shadow-xl border border-white/95">
                {submitted ? (
                  <div className="py-12 text-center space-y-6 animate-in zoom-in-95 duration-300">
                    ...
                    <div className="space-y-2">
                      <h3 className="font-extrabold text-3xl text-slate-900">
                        Consultation Confirmed!
                      </h3>
                      <p className="text-slate-600 text-base max-w-md mx-auto">
                        Thank you, <strong className="text-slate-900">{fullName}</strong>. A calendar invite for <strong className="text-[#007AFF]">{selectedDate} at {selectedTime}</strong> has been sent to <strong className="text-slate-900">{email}</strong>.
                      </p>
                    </div>

                    <div className="p-6 rounded-2xl bg-white/90 max-w-md mx-auto text-left text-xs space-y-2 border border-slate-200/80 shadow-2xs">
                      <div><strong className="text-slate-900 font-bold">Topic:</strong> {selectedTopic}</div>
                      <div><strong className="text-slate-900 font-bold">Organization:</strong> {organization}</div>
                      <div><strong className="text-slate-900 font-bold">Lead Consultant:</strong> {SITE_CONFIG.founder}</div>
                    </div>
                  ...
                  <form onSubmit={handleBooking} className="space-y-8">
                        ...
                        {topics.map((topic) => (
                          <button
                            ...
                            className={`p-3.5 rounded-2xl text-left text-xs font-bold transition-all border cursor-pointer ${
                              selectedTopic === topic
                                ? 'border-[#FF2D55] bg-[#FFEBF0] text-slate-900 shadow-xs'
                                : 'border-slate-200/80 hover:border-slate-300 text-slate-700 bg-white/70'
                            }`}
                          >
                            {topic}
                          </button>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-slate-200/60">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full px-4 py-3 rounded-2xl border border-slate-200/90 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#007AFF]/40 bg-white/80"
                        >
                          {timeSlots.map((slot) => (
                    ...
                    <div className="space-y-4 pt-4 border-t border-slate-200/60">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-xs text-slate-600 mb-1 font-medium">Work Email *</label>
                          <input
                            type="email"
                      ...
                      <div>
                        <label className="block text-xs text-slate-600 mb-1 font-medium">Organization / Hospital / BioPharma *</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-xs text-slate-600 mb-1 font-medium">Key Questions or Goals (Optional)</label>
                        <textarea
                          rows={3}
